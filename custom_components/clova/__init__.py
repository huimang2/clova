...
from .helpers import _get_entity_and_device, get_clova_type, _get_area
...
async def websocket_connect(
    ...
    for state in hass.states.async_all():
        entity_entry, device_entry = await _get_entity_and_device(hass, state.entity_id)
        ...
        default_config[state.entity_id] = {
            ...
            CONF_LOCATION: LOCATION.get(area.name, '') if (area := await _get_area(hass, entity_entry, device_entry)) and area.name else '',
