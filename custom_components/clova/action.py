...
class HealthCheck(_action):
    ...
    async def execute(self, data, params):
        ...
        if state.domain == climate.DOMAIN:
            payload[ATTR_IS_REACHABLE] = state.state in state.attributes.get(climate.ATTR_HVAC_MODES)
            payload[ATTR_IS_TURN_ON] = state.state in [_ for _ in state.attributes.get(climate.ATTR_HVAC_MODES) if _ != climate.HVAC_MODE_OFF]

        else:
            payload[ATTR_IS_REACHABLE] = state.state in [STATE_ON, STATE_OFF]
    ...
    async def pre_process(self, data, params):
        ...
        if state.domain == climate.DOMAIN:
            ...
            if (prefix := data.prefix) == PREFIX_CHANGE:
                ...
                next_mode = FAN_MODES[climate.DOMAIN].get(params[ATTR_DELTA_FAN_SPEED][ATTR_VALUE])
        ...
        # fan 도메인
            ...
            clova_modes = {y: x for x, y in FAN_MODES[fan.DOMAIN].items() if y and y in HA_modes}
            ...
            elif current_mode in fan.OFF_SPEED_VALUES:
                idx = -1
            ...
            if prefix == PREFIX_DECREMENT:
                ...
                next_mode = FAN_MODES[fan.DOMAIN].get(params[ATTR_DELTA_FAN_SPEED][ATTR_VALUE])
...
class GetDeviceState(_action):
    ...
    async def execute(self, data, params):
        ...
        name = (entity_config.get(CONF_NAME) or self.state.name).strip()
...
""" 모드 전환 액션 """      
exec(''.join(BASE_ACTION_TEMPLATE.format(_, "Mode", "True", "{}")
for _ in [PREFIX_SET, PREFIX_CHANGE]))


""" 팬 속도 조절 액션 """  
exec(''.join(BASE_ACTION_TEMPLATE.format(_, "FanSpeed", '''(
((features & climate.SUPPORT_FAN_MODE) and (domain == climate.DOMAIN ))
or ((features & fan.SUPPORT_SET_SPEED) and (domain == fan.DOMAIN ))
...
""" 스윙 모드 액션 """      
exec(''.join(BASE_ACTION_TEMPLATE.format( _, "Oscillation", "(features & climate.SUPPORT_SWING_MODE)", "{}")
